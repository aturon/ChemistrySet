...
class KCASRef[A <: AnyRef](init: A) {
  ...
  private[chemistry] final def getI = (data.get match {
    case (owner: KCAS) => owner.curVal(this)
    case v => v
  }).asInstanceOf[A]

  private [chemistry] final def casI(ov: A, nv: A): Boolean = {
...
private final case class CAS[A <: AnyRef](
  ref: KCASRef[A], ov: A, nv: A
) {
  def execAsSingle: Boolean = ref.casI(ov, nv)
}
