...
object QueueSpec extends Spec {
  class `a queue` {    
    ...
    def `should dequeueAll as nil when empty` {
      var q = new Queue[Integer]()
      q.dequeueAll must have size(0)
    }
    ...
    def `should dequeueAll as nonempty when full` {
      var q = new Queue[Integer]()
      q enqueue 1
      q.dequeueAll.size must be_>(0)
    }
    ...
    def `should dequeue as None after dequeueAll` {
      var q = new Queue[Integer]()
      q enqueue 1
      q enqueue 2
      q.dequeueAll
    ...
    def `should dequeueAll in order` {
      var q = new Queue[Integer]()
      q enqueue 1
      q enqueue 2
      q.dequeueAll must beEqualTo(List(1,2))
    }
