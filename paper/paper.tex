...
\lstinline{read}~(\figref{combinators}), to extract the sentinel value from
...
The \lstinline{cas} combinator~(\figref{combinators}) takes a
...
to CAS is, as discussed in \secref{choice}, a transient failure that causes
the reagent to retry.

%TODO: treat !? as shorthand?

%TODO: note tail recursion

\elide{ The \lstinline{read} combinator has a straightforward signature: if
  \lstinline{r} has type \lstinline{Ref[A]}, then \lstinline{read(r)} is a
  \emph{value} of type \lstinline{Reagent[Unit, A]} which, when invoked, takes
  a unit argument and yields an \lstinline{A} result.
}
...
reaction, returning a result (type \listinline{B}), or fails, returning one of
