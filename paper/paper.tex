...
operations used in isolation (where little overhead for reagentes is expected)
...
related work---especially STM---in~\secref{related}.
...
\lstinline{Reagent[A,B]} with the following signature:\footnote{ Functional
  programming afficianados should regard \lstinline{tryReact} as a kind of
...
In other words, \lstinline{tryReact} takes the input (type \lstinline{A}) to the
reagent and the reaction built up so far, and either completes the reaction,
returning a result (type \listinline{B}), or fails, returning one of the
failure singletons (\lstinline{Block} or \lstinline{Retry}).

\subsection{The entry point: reacting}
\label{sec:impl-reaction}

shown in \figref{reaction}

\elide{
def !(a: A): B = tryReact(a, empty, null) match {
  case (_: Failure) => {
    val backoff = new Backoff
    def retryLoop(shouldBlock: Boolean): B = {
      val offer = if (maySync || shouldBlock) 
                    new Offer[B]
                  else null
      tryReact(a, empty, offer) match {
        case (f: Failure) => {
          if (f == Block && offer != null) 
            LockSupport.park 
          else backoff.once

          // rescind offer, but check for completion
          if (offer != null && offer.tryAbort)
            offer.answer
          else
            retryLoop(f == Block)
        }
...
def !(a: A): B = {
  val backoff = new Backoff
  def retryLoop(shouldBlock: Boolean): B = {
    if (maySync || shouldBlock) {
      val offer = new Offer[B]
      tryReact(a, empty, offer) match {
        case Block => 
          LockSupport.park
          if (offer.rescind) retryLoop(true)
          else offer.answer
        case Retry =>
          backoff.once
          if (offer.rescind) retryLoop(maySync)
          else offer.answer
      }
    } else {
      tryReact(a, empty, null) match {
        case Block => retryLoop(true)
        case Retry => backoff.once; retryLoop(maySync)
        case ans   => ans.asInstanceOf[B]
      }
    }
  retryLoop(false)
...
kcas implementations: \cite{Fraser2007,Luchangco2003,Attiya2008}

\begin{lstlisting}
def tryReact(u: Unit, rx: Reaction, offer: Offer[B]) = 
  if (!rx.hasCAS && !k.hasCAS) {
    if (ref.cas(oldV, newV))
      k.tryReact((), rx, offer)
    else Retry
  } else {
    k.tryReact((), rx.withCAS(ref, oldV, newV), offer)
  }
\end{lstlisting}
...
than lock-freemdom~\cite{Herlihy2003}.  %Work in the area is
...
possible~(\secref{dynamic}).  Like \lstinline{orElse} in Haskell's STM, our
...
elimination backoff stack~(\secref{choice}).
...
guarantees provided thereby), which we discussed in~\secref{stm}.  Aside from
that, we believe that \emph{synchronous} communication is a better choice for
expressing patterns like elimination~(\secref{choice}), since the pattern
relies on both participants being mutually-aware.  %TODO: say more here
