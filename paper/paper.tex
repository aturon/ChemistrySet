...
\elide{
High level concepts:
  Data abstraction/internal normalized syntactic representation
...
\lstinline{BacktrackCommand}, where the latter has just two singleton instances,
...
reagent and the reaction built up so far, and either completes the reaction and
returns its result (type \lstinline{B}) or one of the failure singletons
(\lstinline{Block} or \lstinline{Retry}).
