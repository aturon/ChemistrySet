...
\subsection{Basic reagents: updates, messages, and choice}
...
The first two techniques are built into the combinators.  We introduce a new
type \lstinline{Ref[A]} for atomically updatable references holding data of
type \lstinline{A}.  Unlike Scala's (and Java's) \lstinline{AtomicReference},
\lstinline{Ref} values can only be accessed through the following reagent
combinators:
...
upd:  Ref[A] => PartialFunction[(A,B), (A,C)] 
             => Reagent[B, C]
\end{lstlisting}
By restricting access to the combinators, ... %TODO

% Freedom to change kcas implementations
...
% TODO: fix types for cases below
\begin{lstlisting}
class Counter {
  private val state = new Ref[Int](0)
  val get = upd(state) { case i => (i, i) }
  val inc = upd(state) { case i => (i+1, i) }
  val dec = upd(state) { case i if (i > 0) => (i-1, i) }
  val tryDec = upd(state) {
...
% nice catalysts: zip and unzip

\subsection{Failure is an \textsf{Option}}
\label{sec:failure}

blocking failure versus retryable failure

\begin{lstlisting}
  val tryDec = 
\end{lstlisting}

\subsection{Advanced reagents: CAS, computed, and composed}
...
CML, Transactional events, async CML, join calculus
